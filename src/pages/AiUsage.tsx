...
const AiUsage: React.FC = () => {
    ...
    const parseAgentData = (agentTypeStr?: string) => {
        if (!agentTypeStr) return { type: '-', user: '-', subject: '-' };
        
        let type = agentTypeStr;
        let user = '-';
        let subject = '-';

        if (agentTypeStr.includes('|')) {
            const parts = agentTypeStr.split('|');
            type = parts[0];
            user = parts[1] || '-';
            subject = parts.slice(2).join('|') || '-';
        } else if (agentTypeStr.includes(':')) {
            const parts = agentTypeStr.split(':');
            type = parts[0];
            subject = parts.slice(1).join(':') || '-';
        }

        return { type, user, subject };
    };
                                ...
                                logs.map(log => (
                                    <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="py-3 px-4">
                                            <span className={`px-2 py-1 rounded text-xs font-medium ${parseAgentData(log.agent_type).type === 'web_chat' ? 'bg-purple-100 text-purple-700' : 'bg-orange-100 text-orange-700'}`}>
                                                {parseAgentData(log.agent_type).type === 'web_chat' ? 'Web Chat' : 'Email Draft'}
                                            </span>
                                        </td>
                                        <td className="py-3 px-4 text-sm text-gray-800 truncate max-w-[150px]" title={parseAgentData(log.agent_type).user}>
                                            {parseAgentData(log.agent_type).user}
                                        </td>
                                        <td className="py-3 px-4 text-sm text-gray-800 truncate max-w-[200px]" title={parseAgentData(log.agent_type).subject}>
                                            {parseAgentData(log.agent_type).subject}
                                        </td>
