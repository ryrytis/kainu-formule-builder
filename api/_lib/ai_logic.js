...
export async function processAiMessage(sender_id, message_text, isInternal = false) {
    ...
    try {
        ...
        let baseSystemPrompt = "You are an AI Support Agent for 'Keturi print'. Always be polite and helpful. Do not mention that you are an AI unless asked.\n" +
                               "IMPORTANT RULE 1: PROACTIVE QUOTING IS MANDATORY. If the user asks for a product but doesn't provide enough details (like quantity or lamination), do NOT just ask them clarifying questions empty-handed. Instead, you MUST ASSUME standard specifications (e.g. 100 pcs and 500 pcs) and ASSUME standard sizes (e.g. 90x50 for business cards, DL for gift vouchers) and PROACTIVELY use the 'calculate_price' tool to fetch 'baseline example prices'. Then present these example prices to the client, and warmly ask them to confirm their exact desired quantity and specs. DO NOT guess the price, always use the tool.\n" +
        ...
        if (isInternal) {
            ...
            baseSystemPrompt = "You are an INTERNAL Staff Assistant for 'Keturi print'. You are helping company employees with sensitive data. " +
                               ...
                               "IMPORTANT RULE 1: PROACTIVE QUOTING IS MANDATORY. If the user asks for a product but doesn't provide enough details, do NOT just ask them clarifying questions. Instead, you MUST ASSUME standard specifications (e.g. 100 pcs and 500 pcs) and ASSUME standard sizes, and PROACTIVELY use the 'calculate_price' tool to fetch 'baseline example prices'. Then present these example prices to the user. DO NOT guess the price, always use the tool.\n" +
